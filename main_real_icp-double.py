...
# tvec, rvec
fixed_camera_pose = np.array([0.26809229, 0.68914486, 0.72160278, 0.28006889, -2.68845418, 0.14502297]) # average
# [CAUTION] dim 1-3: tvec, dim 4-6: rvec.

# [CAUTION] Both coordinates in the robot frame: dim 1-3: tvec, dim 4-6: rvec.
    ...
    while True:
        ...
        if key == ord('s'):
            ...
            if res is not None:
                loss, pose = res
                env_pose = real_to_env(pose)
                obs, _ = env2.reset(env_pose, get_agent_env_pos(rtde_r, rtde_c))
