...
from utils.pusht_env import PushTRealEnv
from utils.render_env import *
...
try:
    while True:
        ...
        if res := markerReader.detectMarkers2(color_image, markerDict):
            color_image, ids, rvecs, tvecs, objPoints, allCorners = res
            rvecs = np.array(rvecs)
            tvecs = np.array(tvecs)
            marker_worlds, tblk_envs, objPoints_all = [], [], []
            for rvec, tvec, id in zip(rvecs, tvecs, ids):
                marker_world = pose_tran_fixed_camera(rvec, tvec / 1000, fixed_camera_pose)
                marker_worlds.append(marker_world)
                tblk_env = marker_to_env(marker_world)
                tblk_envs.append(tblk_env)
                objPoints_all.append(objPoints)
            tblk_envs = np.array(tblk_envs)
            marker_worlds = np.array(marker_worlds)
            objPoints_all = np.array(objPoints_all)
            print(tblk_envs)
        ...
        if key == ord('s'):
            print('saving')
            with open('data/homo.pkl', 'wb') as f:
                pickle.dump({'img': color_image_orig, 'img-marked':color_image, 
                             'rvecs': rvecs, 'tvecs': tvecs, 'tblk_envs': tblk_envs, 
                             'marker_worlds': marker_worlds ,'objPoints_all':objPoints_all, 'allCorners':allCorners}, f)
            break
