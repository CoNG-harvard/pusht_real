...
class PushTRealEnv(object):
    ...
    def reset(self, current_env_blk_pose, current_env_agent_pos, seed=None, options=None):
        ...
        #     ])
        state = np.array([
            current_env_agent_pos[0], current_env_agent_pos[1],
            current_env_blk_pose[0], current_env_blk_pose[1],
            current_env_blk_pose[2]
        ])
...
if __name__ == "__main__":
    ...
    env = PushTRealEnv(render_mode='human')
    env.reset(np.array([282, 222, 0]), np.array([316, 294]), options={'shape_type': 'tee'})
    env.render()
    time.sleep(5.0)
    # for i in range(300):
