...
try:
    while True:
        ...
        if res := markerReader.detectMarkers2(color_image, markerDict):
            image, ids, rvecs, tvecs, objPoints, allCorners = res
            for rvec, tvec in zip(rvecs, tvecs):
                cv2.drawFrameAxes(image, cameraMatrix, distortionCoeffs, rvec, tvec, 40)
            
            if len(rvecs) != 4:
                continue
            
            ls = []
            for (rv1, tv1), (rv2, tv2) in combinations(zip(rvecs, tvecs), 2):
                T_marker1_to_camera = rodrigues_to_matrix(rv1, tv1)
                T_marker2_to_camera = rodrigues_to_matrix(rv2, tv2)

                T_marker1_to_marker2 = np.linalg.inv(T_marker2_to_camera) @ T_marker1_to_camera
                tvec = T_marker1_to_marker2[:3, 3]
                rvec = T_marker1_to_marker2[:3, :3]
                ls.append(tvec[-1])
            ls = np.array(ls)
            if np.abs(ls).max() < 15:
                with open('data/homo2.pkl', 'wb') as f:
                    pickle.dump({'image': image, 'image_orig':color_image, "rvecs": rvecs, 
                                 'tvecs':tvecs, 'objPoints': objPoints, 'allCorners': allCorners}, f)
                print('success')
                break
            color_image = image
