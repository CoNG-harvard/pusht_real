...
try:
    while True:
        ...
        color_image_warped = cv2.warpPerspective(color_image, H, (width, height))
        temp = color_image_warped.copy()
        temp[:150, -60:] = 0
        corners = tracker.detect_corners(temp)
        if corners is not None:
            corners = np.concatenate([corners, np.ones((corners.shape[0], 1))], axis=1) @ np.linalg.inv(H).T
            corners = corners[:, :2] / corners[:, 2:]
            for pt in corners:
                cv2.circle(color_image, tuple(pt.astype(int)), 1, (0, 255, 0), -1)
        ...
        if found:
            ...
            print("rvec, tvec marker", rvec, tvec)
